...
from .resource import get_mysql_connection, get_redis_connection
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            for row in results:
                ...
                # 计算所有字段值组合的MD5值
                content = ','.join([x if isinstance(x, str) else str(x)
                                    for x in new_row.values()])
                ...
                if op:
                    v = json.dumps({'id': id, 'op': op,
                                    'data': new_row}, ensure_ascii=False)
                    logger.info('Hit > %s', v)
                    # 将变化数据加入redis队列
            ...
            for k in del_set:
                v = json.dumps(
                    {'id': k.split(':')[1], 'op': 3}, ensure_ascii=False)
                logger.info('Hit > %s', v)
                redis_conn.delete(k)
                redis_conn.lpush(target_table, v)
